...
def getAveAttribute(cluster, dimension):
    """
    Given a Cluster object, finds the average attribute field over the members
    of that cluster.
    
    cluster: the Cluster object to check
    dimension: index for attribute in feature vector
    
    Returns: a float representing the computed average attribute value
    """
    tot = 0.0
    numElems = 0
    for c in cluster.getPoints():
    ...
    for k in kvals:
        ...
        for cluster in clusters:
            for point in cluster.getPoints():
                tErrors.append((point.distance(cluster.getCentroid()))**2)
        tErrorList.append(sum(tErrors))
        ...
        for point in holdoutSet:
            ...
            for i in range(len(clusters)):
                ...
                if distance < smallestDistance:
                    smallestDistance = distance
                    index = i
            hErrors.append((point.distance(clusters[index].getCentroid()))**2) 
        hErrorList.append(sum(hErrors))

    ratio = pylab.array(hErrorList) / pylab.array(tErrorList)

    pylab.figure(1)
    pylab.plot(kvals, tErrorList, label = 'Training set')
    pylab.plot(kvals, hErrorList, label = 'Holdout set')
    pylab.xlabel('k')
    pylab.ylabel('Error')
        ...
        for point in holdoutSet:
            ...
            for i in range(len(clusters)):
                ...
                if distance < smallestDistance:
                    smallestDistance = distance
                    index = i
            aveAttribute = getAveAttribute(clusters[index], dimension)
            errors.append((aveAttribute - point.getOriginalAttrs()[dimension])**2)
        errorList.append(sum(errors) / k)

    pylab.plot(kvals, errorList)
    pylab.xlabel('k')
    pylab.ylabel('Error')
    pylab.show()
...
def problem2(trial):
    for i in range(1, trial + 1):
        print
        print '\tTRIAL', i
        clusters, maxD = kmeans(points, 50, 0.1, County)
        index = None
        for j in range(len(clusters)):
            if clusters[j].contains('TXDallas'):
                index = j
        print "TXDallas is in cluster", index
        counties = ''
        for county in clusters[index].getPoints():
            counties += county.getName() + ', '
        print 'Cluster ' + str(index) + ' contains counties:\n' + counties[:-2]
